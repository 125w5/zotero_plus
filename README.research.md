...
组会新增 **AI 页面与方法图规划 → 核对页面 → 导出可编辑 PPTX 与图形**。DeepSeek 保持默认；实际使用 DSH 提示词组合和工具执行模块。方法图导出 SVG/draw.io，数据图接受带来源的 JSON 数据集。产物引擎安装与输入格式见 [说明](services/research-engine/README.md)。当前用户导出的视觉审查状态明确为待核对，尚未接入图像生成服务或自动视觉审稿。
...
- PDF 中选中文字 → 有道翻译：译文留在阅读器弹出区域。工作台和条目科研侧栏也支持粘贴原文翻译。
...
DOCX 附件自动进入原生全文索引，也可从附件科研侧栏手动重建索引并预览文本。提取正文、标题样式、表格、脚注、尾注、批注及引用指令；不执行域指令、不改写原文件。DOCX 没有可靠固定页码，证据用文件内段落定位。正式编辑由 Word 承担。Windows 自动安装引用插件前检测 Word，没有 Word 时跳过；手动安装保留上游修复入口。
